...
def fig_frontier(df, pool):
    ...
    # Frontier over the seed points, not the condition means. Higher progress
    # and lower cost are both better, so a seed is on the frontier when no
    # other seed beats it on both axes at once. The seed is the unit of
    # replication, so this is the level at which dominance is claimable.
    front = [(p, c) for (p, c) in faint
             if not any((p2 >= p) and (c2 <= c) and ((p2 > p) or (c2 < c))
                        for (p2, c2) in faint)]
    front = sorted(set(front))
    if len(front) > 1:
        ax.plot([p for p, _ in front], [c for _, c in front],
                color="0.45", lw=1.6, ls="--", zorder=3,
                label="frontier over seeds")
