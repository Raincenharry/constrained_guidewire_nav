...
for col, lab, c in [('shadow_max', 'cost from max along device', 'tab:purple'),
                    ('shadow_tip', 'cost from tip (node 61)',    'tab:green')]:
    g = d.groupby('bin', observed=True)[col]
    med = g.median().values
    lo, hi = g.quantile(0.25).values, g.quantile(0.75).values
    x = ctr[:len(med)]
...
fig.tight_layout()
fig.savefig('figs_31_july/figE_cost_signal.png', dpi=200)
print('wrote figs_31_july/figE_cost_signal.png')
